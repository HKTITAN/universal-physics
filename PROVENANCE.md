...
| 🟢 **CH-26(iii) Parts 1–2** — the shell-channel limiting absorption principle (A0-LAP) and the on-band collapse ‖K^b_band q^{1/2}g‖ = O(√b) (the vanishing on-shell amplitude realized as a simple pole-cancelling zero; cold-verified 0.4999) — compressing the free-field gate to the single off-band bound (B) | [iter 27](notes/2026-07-12-iter27-seam-groundwork.md) |
| 🟢 The **branch-(b) infinite-system terminal-rate dichotomy** (ρ_n ≥ |n| / 2+2N−|n| / 2+2N; finite-codimension leading profile) via an inf-convolution fixed point verified in explicit coordinates | [iter 27](notes/2026-07-12-iter27-seam-groundwork.md) |
